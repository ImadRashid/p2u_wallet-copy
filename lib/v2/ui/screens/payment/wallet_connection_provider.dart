...
import '../../../core/models/wallet_connection.dart';
...
class WalletConnectionProvider extends BaseViewModal {
  ...
  WalletConnection? paymentData;

  bool noRequestKey = true;
  ...
  fetchPaymentRequestData() async {
    ...
    var response =
        await _walletAPI.fetchConnectionRequestData(requestKey: requestKey);
    if (!response['error']) {
      paymentData = WalletConnection.fromJson(response["data"]);
    } else {
  ...
  completeRequest() async {
    changeLoading(true);
    var response = await _walletAPI.completeConnectionRequest(
      requestKey: paymentData!.id!,
    );
    changeLoading(false);
    if (!response['error']) {
